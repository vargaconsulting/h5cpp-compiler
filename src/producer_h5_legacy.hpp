...
#ifndef  H5CPP_PRODUCER_H5_LEGACY_HPP
#define  H5CPP_PRODUCER_H5_LEGACY_HPP
...
void LegacyH5Producer::file_begin_impl(){
	io <<
...
void LegacyH5Producer::record_decl_impl(const std::string&var, const std::string& record_name){
	cpp2hid.insert( std::make_pair(var,var));
	io <<"\n"<< indent << "hid_t " << var << " = H5Tcreate(H5T_COMPOUND, sizeof (" << record_name << "));\n";
}

void LegacyH5Producer::return_type_impl(const std::string& var ){
	io << "\n";
...
void LegacyH5Producer::array_decl_impl(const std::string&var,  const std::string& type, uint64_t size){
...
void LegacyH5Producer::type_insert_impl(const std::string& var,  const std::string& field_name,
		const std::string& record_name, const std::string& type){
	io <<indent<<"H5Tinsert(" << var << ", \"" << field_name << "\",\tHOFFSET(" << record_name << "," << field_name << ")," << type <<");\n";
}

void LegacyH5Producer::type_release_impl(){
...
bool LegacyH5Producer::cache_add_impl(const std::string& key, const std::string& type ){
	type_cache.push_back(type);
	cpp2hid.insert( std::make_pair(key,type));
	return 	true;
}
std::string LegacyH5Producer::cache_impl( const std::string& type ){
	return  cpp2hid[type];
}
#endif
